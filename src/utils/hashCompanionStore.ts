...
export const fetchApps = async (accountId: string, privateKey: string) => {
  ...
  try {
    const response = await new ContractCallQuery()
      .setContractId(CONTRACT_ID)
      .setFunction("getAllApps")
      .execute(client);

    // Hedera returns tuple values
    const appsLength = response.getUint256(0).toNumber(); // Example: first item = length
    const apps: string[] = [];

    for (let i = 0; i < appsLength; i++) {
      apps.push(response.getString(i + 1)); // subsequent indices hold strings
    }
